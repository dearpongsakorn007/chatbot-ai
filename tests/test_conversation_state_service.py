...
def test_get_pending_clarification_returns_question_detail_when_found():
    ...
    fake_supabase.table.return_value.select.return_value.eq.return_value.eq.return_value.gt.return_value.limit.return_value.execute.return_value.data = [
        {"question_detail": "เดิมถามเรื่องน้ำมันไฮดรอลิก", "expires_at": "2099-01-01T00:00:00+00:00"}
    ]
    with patch(
    ...
    assert result == "เดิมถามเรื่องน้ำมันไฮดรอลิก"
...
def test_save_pending_clarification_upserts_awaiting_status():
    fake_supabase = MagicMock()
    with patch(
        "app.services.conversation_state_service.get_supabase",
        return_value=fake_supabase,
    ):
        save_pending_clarification("Uabc123", "คำถามเดิม")
